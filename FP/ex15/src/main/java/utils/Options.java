    ...
    public void parseArgs(String[] argv) {
        if (argv != null) {
            for (int argc = 0; argc < argv.length; argc += 2)
                switch (argv[argc]) {
                    case "-d" -> mDiagnosticsEnabled =
                        argv[argc + 1].equals("true");
                    case "-n" -> mNumberOfElements =
                        Integer.parseInt(argv[argc + 1]);
                    case "-p" -> mPrintDiagnosticOnIteration =
                        Integer.parseInt(argv[argc + 1]);
                    case "-t" -> mVirtualThreads =
                        argv[argc + 1].equals("v");
                    default -> {
                        printUsage();
                        return;
                    }
